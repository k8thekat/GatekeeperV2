...
class DB_User(commands.Cog):
    ...
    async def user_steamid(self, context:commands.Context, steam_name:str):
        ...
        await context.send(f'The SteamID of **{steam_name}** is: `{self.uBot.name_to_steam_id(steam_name)}`', ephemeral= True, delete_after= self._client.Message_Timeout)
    ...
    async def user_role(self, context:commands.Context, discord_name:str, role:str):
        ...
        discord_user = self.uBot.userparse(discord_name,context,context.guild.id)
        if discord_user == None:
            await context.send(f'We failed to find the User: {discord_name}, please make sure they are apart of the server..', ephemeral= True, delete_after= self._client.Message_Timeout)
            return

        db_user = self.DB.GetUser(discord_user.id)
        if db_user != None:
            db_user.Role = role
            await context.send(f"We set the User: **{discord_name}** permission's role to `{role}`.", ephemeral= True, delete_after= self._client.Message_Timeout)
        else:
            await context.send(f'We failed to find the User: {discord_name}, please make sure they are in the DB.', ephemeral= True, delete_after= self._client.Message_Timeout)
