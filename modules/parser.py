...
class Parser():
    ...
    def ParseIGNServer(self,messages):
        for msg in messages:
                ...
                if flag_server != None:
                    ...
                    if len(msg_split) > pos+1:
                        ...
                        if end < len(msg_split[pos])-1: 
                            ...
                            if len(possible_server) >= 2:
                                self.logger.debug('possible server attached to server:',possible_server)
                        ...
                        if flag_str_filter != None and len(msg_split) > pos+2:
                            ...
                            if flag_str_filter == None:
                                self.logger.debug('possible server after filter:',possible_server)
                                servers.append(possible_server)
                    
                        else:
                            possible_server = msg_split[pos+1][0:len(msg_split[pos+1])]
                            possible_server = re.sub(self.symbol_reg,"",possible_server)
                            self.logger.debug('possible server:',possible_server) 
                            ...
                            if server == None: 
                                ...
                                self.logger.debug('possible name after server:',possible_name)
                                name = possible_name
                    ...
                    if len(msg_split) > pos+3:
                        ...
                        if flag_symbols == None:
                            ...
                            if flag_str_filter == None:
                                self.logger.debug('next_possible_server:',next_possible_server) #We will attempt to look this server up too!
                ...
                if flag_ign != None or flag_steam != None:
                    ...
                    if len(msg_split) > pos+1:
                        ...
                        if end < len(msg_split[pos])-1: #This catch's IGNs if they are attached to the IGN (eg:IGN:k8thekat)
                            ...
                            possible_name = re.sub(self.symbol_reg, "",possible_name)
                            name_found = True
                            self.logger.debug('possible name attached to IGN:',possible_name)
                            name = possible_name
                        ...
                        if flag_str_filter != None and len(msg_split) > pos+2:
                            ...
                            possible_name = re.sub(self.symbol_reg,"",possible_name)
                            self.logger.debug('possible name after str filter:',possible_name) 
                            name = possible_name
                            ...
                            if name_found:
                                ...
                                self.logger.debug('possible server after name found:', possible_server)
                                servers.append(possible_server)

                            else:
                                possible_name = msg_split[pos+1][0:len(msg_split[pos+1])]
                                possible_name = re.sub(self.symbol_reg,"",msg_split[pos+1])
                                self.logger.debug('possible name:',possible_name) 
                                name = possible_name
                    ...
                    if len(msg_split) > pos+2:
                        ...
                        if flag_search == None and len(msg_split[pos+2]) > 1:
                            ...
                            if possible_name != msg_split[pos+2]: 
                                ...
                                self.logger.debug('possible server after IGN:',possible_server_afterIGN)
                ...
                if flag_whitelist != None:
                    if len(msg_split) > pos+1:
                        ...
                        if flag_str_filter != None and len(msg_split) > pos+2:
                            ...
                            if flag_str_filter == None:
                                self.logger.debug('possible server after whitelist:',possible_server_afterwhitelist)
            ...
            if len(msg_split) < 4:
                flag_server = re.search(self.server_reg, msg)
                flag_ign = re.search(self.ign_reg, msg)
                flag_whitelist = re.search("(whitelist)", msg)
                if flag_ign== None and flag_server== None and flag_whitelist== None:
                    msg_split = msg.split(' ')
                    self.logger.debug('short whitelist request',msg_split)

                    #These CAN and WILL MOST LIKELY FAIL. Very unreliable ways to handling whitelist requests without formatting.
                    self.logger.debug('short possible name:',msg_split[0]) 
                    self.logger.debug('short possible server:', msg_split[1]) 
                    name = msg_split[0]
