...
class AMPInstance():
    ...
    def __init__(self, instanceID=0, serverdata={}, default_console=False, Handler=None):
        ...
        if instanceID == 0 or self.Running:
            if self.check_AMPpermissions(): #We either have to have super or our bot role perms has been set manually!
                ...
                self.setRoleIDs() #This checks for Super Admins role and discord_bot role
    ...
    def check_AMPpermissions(self) -> bool:
    ...
    def _updateInstanceAttributes(self):
        ...
        if len(result["result"][0]['AvailableInstances']) != 0:
            ...
            for i in range(0,len(result["result"][0]['AvailableInstances'])): 
            ...
            for i in range(0,len(result["result"][0]['AvailableInstances'])): #entry = name['result']['AvailableInstances'][0]['InstanceIDs']
                ...
                if instance['InstanceID'] not in amp_instance_keys:
                    ...
                    if instance['DisplayImageSource'] in self.AMPHandler.AMP_Modules:
                        ...
                        server = self.AMPHandler.AMP_Modules[instance['DisplayImageSource']](instanceID= instance['InstanceID'], serverdata= instance, Index= i, Handler= self.AMPHandler)
                        ...
                        server = self.AMPHandler.AMP_Modules['Generic'](instanceID= instance['InstanceID'], serverdata= instance, Index= i, Handler= self.AMPHandler)
                        self.AMPHandler.AMP_Instances[server.InstanceID] = server
                        return instance['FriendlyName']
    ...
    def setRoleIDs(self) -> bool:
        """Sets AMP_BotRoleID and super_AdminID if they exist."""
        result = self.getRoleIds()
        roles = result['result']
        for role in roles:
